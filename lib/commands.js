...
const { diffDirs, syncDirs, listFiles } = require('./diff');
...
async function cmdDecomp(args) {
  ...
  try {
    ...
    // Fonts are synced even though they are not reported, so comp rebuilds the
    // deck with the latest embedded fonts (comp fidelity).
    const sync = existing ? syncDirs(tmp, pptxDir) : changed;
    if (sync.length) {
      log('copying ' + sync.length + ' changed file(s)...');
      copyChanged(sync, tmp, pptxDir);
    }

    if (!flags['no-preview']) {
      const idx = changedSlideIndices(changed);
      if (idx.length) {
        log('rendering ' + idx.length + ' preview(s)...');
        await renderSlides(deck, previewsDir, idx, format, flags.renderer, log);
