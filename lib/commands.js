...
async function cmdDecomp(args) {
  ...
  const dir = positional[1]
    ...
    : path.join(path.dirname(deck), path.basename(deck));
...
For input a.pptx the git-pptx directory is created as a.pptx/ containing:
...
Usage:
  git-pptx decomp <deck.pptx> [dir] [--no-preview] [--format jpg|png]
      split deck.pptx into <deck>.pptx/ (or dir). If the pptx/ subdir already
      holds a previous split, only changed files are updated and previews are
      rendered only for changed slides. Default preview format is jpg.
...
There is no git or GitHub coupling; publish the directory with ordinary git:
  ...
  git add a.pptx && git commit && git push
`;
