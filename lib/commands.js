...
const { planPreviews, commitPreviews } = require('./previews');
...
// PowerPoint stores every XML part on a single line, so a one-character edit
// and a full rewrite are the same diff: one ~150 KB line replaced by another.
// The stored bytes have to stay verbatim - comp must rebuild a package
// PowerPoint accepts, and whitespace inside <a:t> is slide text - so the split
// happens only in what git renders, through a textconv driver. The
// .gitattributes half travels with the directory; the driver itself is a git
// config setting, which cannot be committed, hence the hint decomp prints.
const TEXTCONV_DRIVER = 'pptxml';
const TEXTCONV_CONFIG = 'git config diff.' + TEXTCONV_DRIVER + '.textconv "git-pptx textconv"';

function gitAttributes() {
  return [
    '# Every part is checked out byte for byte. An end-of-line conversion would',
    '# rewrite the whitespace inside <a:t>, and that whitespace is slide text.',
    PPTX_SUB + '/**            -text',
    '',
    '# Readable git diffs for the single-line XML parts PowerPoint writes.',
    '# The driver is a git config setting, so each clone enables it once:',
    '#   ' + TEXTCONV_CONFIG,
    PPTX_SUB + '/**/*.xml     diff=' + TEXTCONV_DRIVER,
    PPTX_SUB + '/**/*.rels    diff=' + TEXTCONV_DRIVER,
    '',
  ].join('\n');
}

// Written once and then left alone, so an edited copy is never clobbered.
function writeGitAttributes(dir) {
  const file = path.join(dir, '.gitattributes');
  if (fs.existsSync(file)) return false;
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(file, gitAttributes());
  return true;
}

function textconvConfigured(cwd) {
  try {
    const v = execFileSync('git', ['config', '--get', 'diff.' + TEXTCONV_DRIVER + '.textconv'],
...
// split, only files that actually changed are updated and only previews that
// no longer match their slide are rendered again.
async function cmdDecomp(args) {
  ...
  try {
    ...
    // directory always holds a package comp can rebuild. `reported` is only
    ...
    if (writeGitAttributes(dir)) log('wrote .gitattributes...');

    if (!flags['no-preview']) {
      // Which previews are out of date is decided from the content each one was
      // rendered at, not from the change set: comp writes a pptx that matches
      // the work dir exactly, so a decomp right after it finds nothing changed
      // while the previews still show the state before the edit.
      const plan = planPreviews(pptxDir, previewsDir, format);
      if (plan.stale.length) {
        // stale holds slide PART numbers (the N in slideN.xml), which is only
        // the part's own creation order, not its position in this deck's slide
        // show. A deck split out of a bigger one keeps its original part
        // numbers, so slide3.xml can be the 1st slide of a 6-slide file.
        // Resolve each part number to its real 1-based position before asking
        // PowerPoint or LibreOffice to export it: otherwise the wrong slide is
        // rendered, and an out-of-range position makes the export fail outright.
        const posMap = deckMod.slidePartToPosition(pptxDir);
        const pairs = plan.stale.map((n) => ({ label: n, position: posMap[n] })).filter((p) => p.position);
        const unresolved = plan.stale.filter((n) => !posMap[n]);
      ...
      const { removed } = commitPreviews(previewsDir, format, plan.wanted);
      if (removed.length) log('removed ' + removed.length + ' preview(s) with no slide...');
    }
    ...
    if (!textconvConfigured(dir)) {
      log('hint: XML parts are stored on one line; readable diffs need the driver once per clone:');
      log('  ' + TEXTCONV_CONFIG);
    }
  } finally {
...
// git calls the driver as `git-pptx textconv <tempfile>` and diffs what it
// writes to stdout. Only the boundary between two adjacent tags is split, so the
// text inside <a:t> keeps the exact spacing the part stores.
function cmdTextconv(args) {
  const { positional } = parseArgs(args);
  if (positional.length < 1) throw new Error('usage: git-pptx textconv <file>');
  const text = fs.readFileSync(positional[0], 'utf8').replace(/(?<=>)(?=<)/g, '\n');
...
Usage:
  git-pptx decomp <deck.pptx> [dir] [--no-preview] [--format jpg|png] [--no-normalize]
      ...
      already holds a previous split, only changed files are updated and only
      previews that no longer match their slide are rendered again. Default
      format is jpg.
  ...
  git-pptx textconv <file>
      print an XML part with a line break between adjacent tags, for git's diff
      driver to read. Not meant to be run by hand.

There is no git or GitHub coupling; commit the deck and the directory with
ordinary git:

  git add a.pptx a.git-pptx && git commit && git push

PowerPoint writes each XML part as a single line, so git shows any edit as one
huge line replaced by another. decomp writes a .gitattributes routing those
parts through a diff driver; enable the driver once per clone:
...
async function run(args) {
  ...
  switch (cmd) {
    ...
    case 'textconv': return cmdTextconv(rest);
    case 'help':
    case '--help':
