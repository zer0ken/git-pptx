...
const VOLATILE = new Set(['docProps/core.xml', 'docProps/app.xml']);
...
// Like diffDirs but also reports volatile files (embedded fonts). Used to sync
// the pptx/ directory so comp rebuilds the deck faithfully; volatile files are
// still excluded from the change report because they are not slide content.
function syncDirs(aDir, bDir) {
  const a = listFiles(aDir);
  const b = listFiles(bDir);
  const changed = [];
  for (const rel of new Set([...a, ...b])) {
...
module.exports = { diffDirs, syncDirs, listFiles, isXml };
