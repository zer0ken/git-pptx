...
function renderLibreOffice(deckPath, outDir, indices, ext, onProgress) {
  ...
  try {
    ...
    indices.forEach((i, k) => {
      if (onProgress) onProgress('rendering slide ' + i + ' (' + (k + 1) + '/' + indices.length + ')');
...
function renderPowerPoint(deckPath, outDir, indices, ext, onProgress) {
  fs.mkdirSync(outDir, { recursive: true });
...
function renderSlides(deckPath, outDir, indices, format, renderer, onProgress) {
  const ext = format === 'png' ? 'png' : 'jpg';
  let backend = renderer || 'auto';
  if (backend === 'auto') {
    backend = (process.platform === 'win32' && hasPowerPoint()) ? 'powerpoint' : 'libreoffice';
  }
  if (backend === 'powerpoint') renderPowerPoint(deckPath, outDir, indices, ext, onProgress);
  else if (backend === 'libreoffice') renderLibreOffice(deckPath, outDir, indices, ext, onProgress);
