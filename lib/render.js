...
function renderLibreOffice(deckPath, outDir, pairs, ext, onProgress) {
  ...
  try {
    ...
    pairs.forEach(({ label, position }, k) => {
      if (onProgress) onProgress('rendering slide ' + label + ' (' + (k + 1) + '/' + pairs.length + ')');
...
function renderPowerPoint(deckPath, outDir, pairs, ext, onProgress) {
  fs.mkdirSync(outDir, { recursive: true });
...
// of a larger deck.
function renderSlides(deckPath, outDir, pairs, format, renderer, onProgress) {
  const ext = format === 'png' ? 'png' : 'jpg';
  let backend = renderer || 'auto';
  if (backend === 'auto') {
    backend = (process.platform === 'win32' && hasPowerPoint()) ? 'powerpoint' : 'libreoffice';
  }
  if (backend === 'powerpoint') renderPowerPoint(deckPath, outDir, pairs, ext, onProgress);
