...
function stripDateFields(node) {
  if (node.nodeType !== 1) return;
  // PowerPoint auto-refreshes date fields on save, so the field text is
  // derived noise; ignore it for comparison.
  if (node.nodeName === 'a:fld' && /datetime|date/i.test(node.getAttribute('type') || '')) {
    const drop = [];
    for (let c = node.firstChild; c; c = c.nextSibling) {
      if (c.nodeType === 1 && c.nodeName === 'a:t') drop.push(c);
    }
    drop.forEach((n) => node.removeChild(n));
  }
  for (let c = node.firstChild; c; c = c.nextSibling) stripDateFields(c);
}

function canonicalize(xml) {
  const doc = new DOMParser().parseFromString(xml, 'text/xml');
  const root = doc.documentElement;
  if (root) stripDateFields(root);
